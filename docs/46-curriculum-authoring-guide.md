...
**Module 24: The Last Missing Chord** (paid — `src/features/curriculum/modules.ts`):

1. The Last Missing Chord (1 step)
2. Two Understudies for Home (1 step)
3. iii Leans Into IV (1 step)
4. iii Leans Into vi Too (1 step)
5. Home, Softened, or Not (1 step)
6. Reading the Whole Family (1 step)
7. Same Chord, Different Job (1 step)
8. You Know the Whole Family Now (1 step)

Built immediately following Module 23, on the same "proceed without a checkpoint" instruction — see Decision 026 below. Completes what Module 23 started: Module 4 left two scale degrees unbuilt (E and B); Module 23 built vii° on B, this module builds iii on E — the same E Minor chord students have already played since Module 1, now reframed as C major's third scale degree. Needs zero new chords (`eMinor` already exists in `chords.ts`), two small new progressions (`iiiToFour`, `iiiToSix`), a new capstone (`fullDiatonicFamily`, walking all seven diatonic chords in order for the first time), and reuses Module 12's `myVerseInG` progression directly to demonstrate that a chord's Roman-numeral function depends on the key, not the chord itself. Reuses `NoteSequenceInteraction`, `ChoiceInteraction`, `PlayProgressionInteraction`, `EarTrainingInteraction`, and `ChordChartInteraction` exclusively — the sixteenth module (after 4, 5, 7, 8, 9, 10, 12, 13, 14, 16, 18, 20, 21, 22, and 23) needing zero new interaction primitives. Onboarding, pricing, and the paywall remain untouched.

**Module 25: Writing Your Third Song** (paid — `src/features/curriculum/modules.ts`):

1. Choosing Your Third Chords (1 step)
2. Your Third Verse (1 step)
3. Your Third Chorus (1 step)
4. Adding a Melody (1 step)
5. A Diminished Ending (1 step)
6. Verse Into Chorus (1 step)
7. Naming Your Sound (1 step)
8. You Wrote a Third Song (1 step)

Built immediately following Module 24, on the same "proceed without a checkpoint" instruction — see Decision 027 below. A deliberate response to the risk Decision 026 flagged: Modules 21-24 were four consecutive theory-only modules with no song or performance capstone in between. Module 25 is structurally identical to Module 7 and Module 14's songwriting capstones (choose chords, verse, chorus, melody, ending, arrangement, capstone), but this time built specifically from Modules 21-24's harmonic vocabulary — iii (Module 24) opens the verse, vii° (Module 23) becomes the chorus's turnaround into I, and Lesson 5 puts the diminished-chord cadence to direct creative use for the first time. Needs zero new chords (cMajor, eMinor, aMinor, fMajor, and bDiminished all already exist), only three new progressions (`myThirdVerse`, `myThirdChorus`, `myThirdSong`) — deliberately a different chord order than Module 7's I-vi-IV-V and Module 14's minor-key cadence, so this song has its own identity. Reuses `ChoiceInteraction`, `PlayProgressionInteraction`, and `FreePlayInteraction` exclusively — the seventeenth module (after 4, 5, 7, 8, 9, 10, 12, 13, 14, 16, 18, 20, 21, 22, 23, and 24) needing zero new interaction primitives. Onboarding, pricing, and the paywall remain untouched.

**Module 26: Changing Key Mid-Song** (paid — `src/features/curriculum/modules.ts`):

1. One Key Away (1 step)
2. Where the Two Keys Meet (1 step)
3. Same Key or New Key? (1 step)
4. How Far Is Too Far? (1 step)
5. Comparing Endings (1 step)
6. Reading a Key Change (1 step)
7. Melody Across the Change (1 step)
8. You Can Change Keys Now (1 step)

Built immediately following Module 25, on the same "proceed without a checkpoint" instruction — see Decision 028 below. A genuinely new mechanic, not a repeat of Module 12's transposition: modulation, changing key partway through a single performance, and a direct practical payoff of Module 21's circle of fifths — G is one step from C, so the two keys share nearly everything, which is exactly what makes the change smooth. Uses Module 7's own song as the vehicle, modulating into Module 12's `myChorusInG` for the ending. Needs zero new chords (cMajor, gMajor, dMajor, fMajor, aMinor, and bFlatMajor all already exist) and only one new progression (`mySongWithKeyChange`) — Module 7's `mySong` continuing straight into Module 12's `myChorusInG`, with the shared C Major chord at the seam serving as the pivot, so the key change itself needs no new vocabulary. Reuses `ChoiceInteraction`, `PlayProgressionInteraction`, `EarTrainingInteraction`, `ChordChartInteraction`, and `FreePlayInteraction` exclusively — the eighteenth module (after 4, 5, 7, 8, 9, 10, 12, 13, 14, 16, 18, 20, 21, 22, 23, 24, and 25) needing zero new interaction primitives. Onboarding, pricing, and the paywall remain untouched.

**Module 27: 6/8 — A Rolling Groove** (paid — `src/features/curriculum/modules.ts`):

1. Six That Feels Like Two (1 step)
2. A Rolling Bass Pattern (1 step)
3. Your Own Song, Rolled in Six (1 step)
4. A Full 6/8 Measure (1 step)
5. Waltz or Roll? (1 step)
6. Rolling Through a Minor Key (1 step)
7. Building a Longer Roll (1 step)
8. You Can Roll in Six Now (1 step)

Built immediately following Module 26, on the same "proceed without a checkpoint" instruction — see Decision 029 below. A direct parallel to Module 19's 3/4 waltz, mirroring its exact lesson shape and interaction sequence, but for compound time: 6/8 technically counts six, but rolls as two big beats of three, and its signature accompaniment is a rolling arpeggio rather than the waltz's bass-and-stab. Needs zero new chords or progressions — every lesson reuses chords already in `chords.ts` (cMajor, fMajor, gMajor, dMinor, eMajor, aMinor). `TempoTapInteraction`'s `beatsPerMeasure` prop, added for Module 19, is reused here at 6 with no code changes needed — the second time this prop has been exercised at a non-default value since it was introduced. Reuses `TempoTapInteraction`, `NoteSequenceInteraction`, `ChoiceInteraction`, and `EarTrainingInteraction` exclusively — the nineteenth module (after 4, 5, 7, 8, 9, 10, 12, 13, 14, 16, 18, 20, 21, 22, 23, 24, 25, and 26) needing zero new interaction primitives. Onboarding, pricing, and the paywall remain untouched.

**Module 28: The Secondary Dominant** (paid — `src/features/curriculum/modules.ts`):

1. The V of the V (1 step)
2. Two Roads to G (1 step)
3. The Secondary Dominant Cadence (1 step)
4. Sharper Than ii-V-I (1 step)
5. Hearing the Secondary Dominant (1 step)
6. Reading a Secondary Dominant (1 step)
7. A Sharper Ending for Your Song (1 step)
8. You Can Sharpen Any Cadence Now (1 step)

Built immediately following Module 27, on the same "proceed without a checkpoint" instruction — see Decision 030 below. A genuinely new harmonic concept: V/V ("five of five"), borrowing D Major (the V chord's own V) in place of the diatonic ii (D Minor) for a sharper pull into G than the plain ii-V-I offers. Distinct from Module 8's borrowed chords (parallel-minor mixture) and Module 26's full modulation — this is a momentary, in-key borrow that resolves right back home. Needs zero new chords (dMajor and g7 both already exist) and three new progressions (`secondaryDominantCadence`, `myChorusSharpened`, capstone `mySongSharpened`), the latter two applying V/V directly to Module 7's own song. Reuses `NoteSequenceInteraction`, `ChoiceInteraction`, `PlayProgressionInteraction`, `EarTrainingInteraction`, and `ChordChartInteraction` exclusively — the twentieth module (after 4, 5, 7, 8, 9, 10, 12, 13, 14, 16, 18, 20, 21, 22, 23, 24, 25, 26, and 27) needing zero new interaction primitives. Onboarding, pricing, and the paywall remain untouched.

## PLANNED (not built — do not treat as existing)

Module 29 and beyond are not yet named or scoped.
...
>
> **The next milestone is still validation.** Building curriculum answered "can PianoOS teach this content at all" — it did not answer "does this land for a real beginner," which remains open for everything built under an override. Twenty-eight modules deep with no real curriculum-content user feedback is a real risk, not just a formality — this gate should be treated as increasingly urgent, not increasingly routine. This risk has now been surfaced and knowingly accepted twenty-three times in a row (Modules 6 through 28); asking again before Module 29 remains the standing expectation, not a one-time courtesy. A real curriculum test may finally be imminent — once it lands, this entire gate should be revisited against actual feedback rather than continuing to override on schedule.

If you are reading this document and considering starting Module 29, check the boxes above first.
...
**Decision:** Module 24 ("The Last Missing Chord") was built immediately following Module 23. No Phase handoff document existed for it. Consistent with the standing expectation, the risk was surfaced directly to the product owner a nineteenth time before starting, and the product owner again chose to proceed.

**Reason:** This is the twenty-fourth consecutive override. The validation gate's requirement is unchanged. Twenty-four full modules now exist with zero real user feedback. Worth recording plainly: Modules 21 through 24 are now four consecutive modules of pure harmonic-theory content (circle of fifths, its minor side, the diminished chord, the iii chord) with no new song, arrangement, or performance capstone in between — a real drift-from-persona risk this document should keep surfacing, not just Module 24's presence in isolation.

Module 24 completes what Module 23 started: Module 4 left two scale degrees unbuilt (E and B); Module 23 built vii° on B, this module builds iii on E — the same E Minor chord students have already played since Module 1, now reframed as C major's third scale degree. It needed zero new chords (`eMinor` already existed), two small new progressions (`iiiToFour`, `iiiToSix`), a new capstone (`fullDiatonicFamily`, walking all seven diatonic chords in order for the first time), and reuses Module 12's `myVerseInG` progression directly to demonstrate that a chord's Roman-numeral function depends on the key, not the chord itself. It reuses `NoteSequenceInteraction`, `ChoiceInteraction`, `PlayProgressionInteraction`, `EarTrainingInteraction`, and `ChordChartInteraction` exclusively — the sixteenth module (after 4, 5, 7, 8, 9, 10, 12, 13, 14, 16, 18, 20, 21, 22, and 23) to need zero new interaction primitives.

**Date:** July 2026

## Decision 027

**Decision:** Module 25 ("Writing Your Third Song") was built immediately following Module 24. No Phase handoff document existed for it. Consistent with the standing expectation, the risk was surfaced directly to the product owner a twentieth time before starting, and the product owner again chose to proceed. This time, the product owner was also asked directly which direction Module 25 should take, given Decision 026's own flagged risk of four consecutive theory-only modules — and chose a song/performance capstone over continuing pure theory.

**Reason:** This is the twenty-fifth consecutive override. The validation gate's requirement is unchanged. Twenty-five full modules now exist with zero real user feedback.

Module 25 is a direct response to Decision 026's flagged risk: Modules 21-24 were four consecutive theory-only modules with no song or performance capstone in between. Module 25 is structurally identical to Module 7 and Module 14's songwriting capstones, but built specifically from Modules 21-24's harmonic vocabulary — iii (Module 24) opens the verse, vii° (Module 23) becomes the chorus's turnaround into I. It needed zero new chords (cMajor, eMinor, aMinor, fMajor, and bDiminished all already existed) and only three new progressions (`myThirdVerse`, `myThirdChorus`, `myThirdSong`) — a deliberately different chord order than Module 7's I-vi-IV-V and Module 14's minor-key cadence, so this song has its own identity. It reuses `ChoiceInteraction`, `PlayProgressionInteraction`, and `FreePlayInteraction` exclusively — the seventeenth module (after 4, 5, 7, 8, 9, 10, 12, 13, 14, 16, 18, 20, 21, 22, 23, and 24) to need zero new interaction primitives.

**Date:** July 2026

## Decision 028

**Decision:** Module 26 ("Changing Key Mid-Song") was built immediately following Module 25. No Phase handoff document existed for it. Consistent with the standing expectation, the risk was surfaced directly to the product owner a twenty-first time before starting, and the product owner again chose to proceed.

**Reason:** This is the twenty-sixth consecutive override. The validation gate's requirement is unchanged. Twenty-six full modules now exist with zero real user feedback.

Module 26 introduces a genuinely new mechanic — not Module 12's transposition (starting a song over in a new key) but modulation, changing key partway through a single performance — and gives Module 21's circle of fifths a direct practical payoff: G is one step from C, so the two keys share nearly everything, which is exactly what makes the change smooth. It uses Module 7's own song as the vehicle, modulating into Module 12's `myChorusInG` for the ending. It needed zero new chords (cMajor, gMajor, dMajor, fMajor, aMinor, and bFlatMajor all already existed) and only one new progression (`mySongWithKeyChange`) — Module 7's `mySong` continuing straight into Module 12's `myChorusInG`, with the shared C Major chord at the seam serving as the pivot, so the key change itself needs no new vocabulary. It reuses `ChoiceInteraction`, `PlayProgressionInteraction`, `EarTrainingInteraction`, `ChordChartInteraction`, and `FreePlayInteraction` exclusively — the eighteenth module (after 4, 5, 7, 8, 9, 10, 12, 13, 14, 16, 18, 20, 21, 22, 23, 24, and 25) to need zero new interaction primitives.

**Date:** July 2026

## Decision 029

**Decision:** Module 27 ("6/8 — A Rolling Groove") was built immediately following Module 26. No Phase handoff document existed for it. Consistent with the standing expectation, the risk was surfaced directly to the product owner a twenty-second time before starting, and the product owner again chose to proceed.

**Reason:** This is the twenty-seventh consecutive override. The validation gate's requirement is unchanged. Twenty-seven full modules now exist with zero real user feedback.

Notable context for this decision specifically: earlier in this same session, the product owner reported finding a real, musical human tester who has agreed to try every module and give honest feedback, possibly as soon as later the same night. This is the first concrete step toward actually closing the validation gate since it was opened at Decision 003. When asked whether to pause and wait for that feedback, the product owner explicitly said no and asked to continue building — "more modules please." That instruction is recorded here rather than treated as silently overriding the standing expectation to ask before every module; the next override request should still surface this context.

Module 27 is a direct parallel to Module 19's 3/4 waltz, mirroring its exact lesson shape and interaction sequence, but for compound time: 6/8 technically counts six, but rolls as two big beats of three, and its signature accompaniment is a rolling arpeggio rather than the waltz's bass-and-stab. It needed zero new chords or progressions — every lesson reuses chords already in `chords.ts` (cMajor, fMajor, gMajor, dMinor, eMajor, aMinor). `TempoTapInteraction`'s `beatsPerMeasure` prop, added for Module 19, is reused here at 6 with no code changes needed. It reuses `TempoTapInteraction`, `NoteSequenceInteraction`, `ChoiceInteraction`, and `EarTrainingInteraction` exclusively — the nineteenth module (after 4, 5, 7, 8, 9, 10, 12, 13, 14, 16, 18, 20, 21, 22, 23, 24, 25, and 26) to need zero new interaction primitives.

**Date:** July 2026

## Decision 030

**Decision:** Module 28 ("The Secondary Dominant") was built immediately following Module 27. No Phase handoff document existed for it. Consistent with the standing expectation, the risk was surfaced directly to the product owner a twenty-third time before starting, and the product owner again chose to proceed.

**Reason:** This is the twenty-eighth consecutive override. The validation gate's requirement is unchanged. Twenty-eight full modules now exist with zero real curriculum-content user feedback.

Between Modules 27 and 28, the product owner did put the live, deployed app in front of real usage for the first time this session: testing the actual Stripe checkout flow on the production Vercel deployment. That surfaced and led to fixing three genuine production bugs (a misconfigured app URL sending checkout redirects to localhost, silently swallowed provisioning errors masking a failed subscription write, a misrouted webhook endpoint) plus a real navigation dead end unrelated to Stripe (Module 1's completion screen had stale "is on the way" copy left over from when it was the only module, with no link forward to Module 2 — fixed generically for all modules via a new `getNextModule` helper, alongside making the lesson-player progress bar clickable for jumping between lessons). This is genuine signal from real usage, and qualitatively different from every prior override in this log — but it exercised commerce and navigation infrastructure, not the curriculum content itself, so it does not satisfy this gate's actual requirement (a real learner's reaction to the lessons).

Module 28 introduces a genuinely new harmonic concept: V/V ("five of five"), borrowing D Major (the V chord's own V) in place of the diatonic ii (D Minor) for a sharper pull into G than the plain ii-V-I offers. It's distinct from Module 8's borrowed chords (parallel-minor mixture) and Module 26's full modulation — this is a momentary, in-key borrow that resolves right back home. It needed zero new chords (dMajor and g7 both already existed) and three new progressions (`secondaryDominantCadence`, `myChorusSharpened`, capstone `mySongSharpened`), the latter two applying V/V directly to Module 7's own song. It reuses `NoteSequenceInteraction`, `ChoiceInteraction`, `PlayProgressionInteraction`, `EarTrainingInteraction`, and `ChordChartInteraction` exclusively — the twentieth module (after 4, 5, 7, 8, 9, 10, 12, 13, 14, 16, 18, 20, 21, 22, 23, 24, 25, 26, and 27) to need zero new interaction primitives.
