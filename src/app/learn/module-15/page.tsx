...
import { MODULE_15, MODULE_15_ID } from "@/features/curriculum/modules";
...
export default async function Module15Page() {
  ...
  if (!user) {
    redirect("/login?redirectTo=/learn/module-15");
  }
  ...
  return (
    <div className="mx-auto w-full max-w-2xl px-4 py-14 sm:px-6 md:py-20">
      ...
      <p className="mt-1 text-muted-foreground">{MODULE_15.subtitle}</p>

      <div className="mt-10">
        <LessonList
          lessons={MODULE_15.lessons}
          moduleId={MODULE_15_ID}
          basePath="/learn/module-15"
          completionCopy={{
            heading: "You've completed Module 15.",
            body: "No staff, no individual notes — just chord names, in order. You can read anything now.",
          }}
        />
      </div>
    </div>
