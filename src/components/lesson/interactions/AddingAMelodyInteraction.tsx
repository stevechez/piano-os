...
const VERSE = getProgression("myVerse");
const BACKING = VERSE.chordIds.map((id) => getChord(id).notes);

export function AddingAMelodyInteraction({ onComplete }: LessonInteractionProps) {
  return (
    <FreePlayInteraction
      ...
      prompt={(played, total) =>
        played >= total
          ? "That melody, over your own verse, is part of your song now."
